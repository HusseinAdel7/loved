...
function initAudioEngine() {
 let audio = document.getElementById("global-romantic-music");
 if (!audio) {
 ...
 const currentPath = window.location.pathname;
 const folders = ["/index/", "/love-story/", "/gallery/", "/countdown/", "/reasons/", "/surprise/"];
 const isSubFolder = folders.some(f => currentPath.includes(f));
 const localPath = isSubFolder ? "../assets/music/romantic-bg.mp3" : "./assets/music/romantic-bg.mp3";
 
 // Use direct HTTPS CDN link to bypass local browser security (CORS) blocks on file:// protocol
 const assetPath = window.location.protocol === "file:" 
  ? "https://raw.githubusercontent.com/BaseMax/happy-birth-letter/main/happy-birthday.mp3"
  : localPath;
 
 if (audio.src !== assetPath) {
  audio.src = assetPath;
 }
 ...
 // Exciting fallback Synth in case MP3 file fails to load at all
 const fallbackSynth = {
  gainNode: null,
  isPlaying: false,
  timer: null,
